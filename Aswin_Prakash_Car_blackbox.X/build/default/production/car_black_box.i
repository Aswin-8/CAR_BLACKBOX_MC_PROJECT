...
unsigned char menu_screen(unsigned char key)
{
    ...
    else if(key==0x3F && flag1==1 && menu_pos < 4 && delay!=0)
    {
        clear_screen();
        menu_pos++;
        long_press_delay=0;
   ...
   else if(key == 0x3F && menu_pos > 0 && flag2==1 && delay!=0)
    {
        clear_screen();
        menu_pos--;
        long_press_delay=0;
...
void download_log(void)
{
    ...
    while(index1 <= pos)
    {
        ...
        {
            download_data[z-(5+(10*index1))]=read_externaleeprom(z);
        }
...
unsigned char set_time(unsigned char key)
{
    ...
    else if(key==0x3F && flag5==1 && time_pos < 3 && delay1!=0)
   {
        time_pos++;
        if(time_pos==3)
            time_pos=0;
    ...
    else if(key==0x3F && flag6==1 && sw_flag==0 && delay1!=0)
    {
        sw_flag=1;
        flag6=0;
        delay1=0;
    ...
    else if(time_pos==2)
    {
        ...
        clock_reg[1]=((time[2] & 0x07) << 4) | (time[3] & 0x0F);
...
void view_log(unsigned char key)
{
    ...
    {
      log_data[k-(5+(10*index))]=read_externaleeprom(k);
    }
    clcd_print ("# TIME     E  SP",(0x80 + 0));
    clcd_putch (index+'0',(0xC0 + 0));
    clcd_putch (log_data[0],(0xC0 + 2));
    clcd_putch (log_data[1],(0xC0 + 3));
    clcd_putch(':',(0xC0 + 4));
    clcd_putch (log_data[2],(0xC0 + 5));
    clcd_putch (log_data[3],(0xC0 + 6));
    clcd_putch(':',(0xC0 + 7));
    clcd_putch (log_data[4],(0xC0 + 8));
    clcd_putch (log_data[5],(0xC0 + 9));
    clcd_putch (log_data[6],(0xC0 + 11));
    clcd_putch (log_data[7],(0xC0 + 12));
    clcd_putch (log_data[8],(0xC0 + 14));
    clcd_putch (log_data[9],(0xC0 + 15));
}
...
unsigned char change_password(unsigned char key,unsigned char reset_flag)
{
    ...
    else if(p == 4 && once==0)
    {
        ...
        clcd_write(0x0F, 0);
        _delay((unsigned long)((100)*(20000000/4000000.0)));
        clcd_print ("Re Enter new Pas",(0x80 + 0));
    }
    if(key == 0x37 && q <4 && once==2)
    {
        re_passwd[q]='1';
        q++;
        clcd_putch('*',(0xC0 + q+4));
    }
    else if (key == 0x2F && q <4 && once==2)
    {
        re_passwd[q]='0';
        q++;
        clcd_putch('*',(0xC0 + q+4));
    ...
    {
        p=0,q=0;
        if(strncmp(re_passwd,new_passwd,4)==0)
        {
            clear_screen();
            clcd_write(0x0C, 0);
            _delay((unsigned long)((100)*(20000000/4000000.0)));
...
void log_event_eeprom()
{
    ...
    addr= 5 + (10*pos);
    str_write_externaleeprom(addr,log);
}
...
unsigned char login_screen(unsigned char key,unsigned char reset_flag)
{
    ...
    if(i == 4)
    {
        ...
        if(strncmp(user_passwd,spasswd,4)==0)
        {
            clear_screen();
            clcd_write(0x0C, 0);
             _delay((unsigned long)((100)*(20000000/4000000.0)));
        ...
        else
        {
            ...
            else
            {
                ...
                clcd_putch(attempt_rem +'0',(0xC0 + 14));
                _delay((unsigned long)((2000)*(20000000/4000.0)));
            }
            clear_screen();
